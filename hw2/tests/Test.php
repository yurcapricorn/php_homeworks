...
abstract class Test
{
    ...
    public static function classConfigTest(){
        ...
        if($config!==$config2){echo 'Singleton error';}
        if($config->data['db']['host']==null){echo 'ConfigTest Error';};
    }
    ...
    public static function modelInsertMethodTest(){
        ...
        $article->title='test';
        $article->save();
        if($article->id==null){echo 'modelInsertMethodTest error';}
    }
    ...
    public static function modelUpdateMethodTest(){
        ...
        $article->id='100';
        $res=$article->save();
        if(!$res){echo 'modelUpdateMethodTest error';}
    }
